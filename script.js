$(document).ready(function () {
  ...
  function createRows() {
    for (var i = 0; i < hourArray.length; i++) {
      ...
      var newRow = $("<div>")
        .addClass("row d-flex time-block past")
        .attr("id", militaryHoursArray[i]);

      newRow.append(
        $("<div>" + hourArray[i] + "</div>").addClass("col-2 hour")
      );

      newRow.append(
        $(
          "<textarea placeholder='Add Event'>" + savedText + "</textarea>"
        ).addClass("col-9")
      );

      newRow.append(
        $(
          "<button type='button submit' class='btn btn-primary saveBtn col-1'></button>"
        ).append("<i class='far fa-save fa-2x'></i>")
      );
  ...
  $("#clear-events").on("click", function (event) {
    ...
    var clearAll = confirm(
      "Are you sure you would like to clear ALL of the events?"
    );
    if (clearAll === true) {
